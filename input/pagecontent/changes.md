...
**August 10, 2026** — Article 5 patient-provided data: added the [Patient-Provided Data](patient-provided-data.html) functional page specifying the document-based path for the EHDS Article 5 insertion right (ITI-105 via Document Publisher and Document Submission Option). Distinguishability marking (`author` = Patient/RelatedPerson, `securityLabel` `PATRPT`/`SDMRPT`, `meta.source`), non-alteration rules (new-document-only submissions; replacement/update/removal restricted to the person's own prior submissions), representative (Art. 4(2)) and wellness application (Art. 48(2)) handling, and an informative layered security model. Added an Article 5 anchor section and IG-originated requirement rows (`api-publisher-patientInsert`, `api-publisher-patientDistinguish`, `api-provider-patientDistinguish`, `api-provider-patientNoAlter`) to Regulatory Anchors. Converted the informative Art. 5 sections on the Health Data Access Service and Wellness Apps pages to pointers. Added patient-provided and representative-provided DocumentReference examples. Added open issue 13 (Article 5 marking) for ballot feedback.
