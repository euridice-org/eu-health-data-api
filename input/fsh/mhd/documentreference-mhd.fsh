...
**Search Strategy**:
- `type`: precise document type for discovery. `DocumentTypeEuApiVS` (LOINC) is **preferred**, so broader LOINC or other type codings stay conformant. See [Document Exchange](document-exchange.html) for type codes per EHDS priority category.
- `category`: broad classification for coarse discovery. Bound **extensible** to `EhdsPriorityCategoryEuApiVS` (LOINC Document Class) for the EHDS context; broader codings remain conformant.
...
* category 0..1
* category MS
* category from EhdsPriorityCategoryEuApiVS (extensible)
* type MS
