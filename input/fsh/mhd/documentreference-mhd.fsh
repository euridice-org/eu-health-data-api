...
- `category`: the coarse **document class** (XDS `classCode`), for server-side query filtering ("all the lab documents"). Bound **(preferred)** to the LOINC-based FHIR [Document Class value set](https://hl7.org/fhir/R4/valueset-document-classcodes.html) — any document-class code is valid; additional codings (SNOMED, XDS) MAY be used. For EHDS cross-border discovery, the priority-category subset of class codes is listed (informatively) in [EEHRxFDocumentCategoryVS](ValueSet-eehrxf-document-category-vs.html) and correlated to the regulatory categories via the [EehrxfMhdDocumentReferenceCM](ConceptMap-EehrxfMhdDocumentReferenceCM.html) ConceptMap. `category` stays 0..1 — a document has one class; other classification axes use `setting`/`facility`/`event`.
...
* category MS
* category from $document-classcodes (preferred)
* type MS
